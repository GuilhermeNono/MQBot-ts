...
async function Report(message, client) {
    try {
        if (message.guild.id !== "929417995325956177")
            return;
        //*1 Pegando as informações do canal.
        let channelReports = message.guild.channels.cache.get("929428196313669692");
        if (channelReports.type === "GUILD_TEXT") {
            //*2 Verificando se a mensagem não é do bot e se foi enviada no canal correto.
            if (message.channel.id !== channelReports.id)
                return;
            if (message.deletable)
                message.delete();
            //*3 Verificando se a mensagem não passou dos 4096 caracteres(Limite de caracteres informado pela api)
            let limitDescriprion = new discord_js_1.MessageEmbed()
                .setColor("#fcb83a")
                .setTitle("Limite excedido.")
                .setDescription("Aparentemente, o limite de caracteres por report foi excedido. Por favor, limite seu report a no maximo **4096 Caracteres**.");
            ...
            let embedReport = new discord_js_1.MessageEmbed()
                .setTitle(":recycle: | SEU TICKET FOI ENVIADO COM SUCESSO | :recycle:")
                .setDescription(" Podemos em breve entrar em contato para mais detalhes ou resolver por tras dos panos. Independente da modo, você será notificado quando for resolvido.")
            ...
            let embedReportModeration = new discord_js_1.MessageEmbed()
                .setColor("#fc6060")
                .setTitle(`:question: | Report de ${message.author.tag} ➔ ` +
                message.guild.name +
                " |:question: ")
            ...
            function SendEmbed(msg) {
                const peachServer = client.guilds.cache
                ...
                const briocoServer = client.guilds.cache
