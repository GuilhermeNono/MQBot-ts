...
class ExtendedClient extends Client {
  ...
  public async init() {
    ...
    readdirSync(commandPath).forEach((dir) => {
      ...
      console.log(green.bold(`✔ ${dir} - Commands loaded without any error. ✔`));
      } catch (error) {
        console.log(red.bold(`❌ LOADING FAILURE - Failed to load folder "${dir ?? __dirname}". ❌`))
        throw error
      }
    });
    ...
    const load_dir = async (dir: string) => {
      try {
        ...
        console.log(green.bold(`✔ ${dir} - Events loaded without any error. ✔`));
