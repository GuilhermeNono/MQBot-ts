...
export const command: Command = {
  name: "giveinsignia",
  aliases: ["giveins", "gi"],
  description: "Dá uma insignia a um usuário.",
  run: async (client, message, args) => {
    ...
    try {
      ...
      let insignias: string | string[] = message.content.split(" ").splice(2).join(" ");
      if (insignias === "") return message.channel.send(`" .giveinsignia ${person.id} **➔${insignias}** " não é um parametro valido.`);
      ...
      insignias.forEach(async ins => {
          ...
          if(insTest === null) return message.channel.send(`" .giveinsignia ${person.id} **➔${ins}** " não é um parametro valido.`);

          if(!memberDB.insigniaID.includes(parseInt(ins))) {
            memberDB.insigniaID.push(parseInt(ins));
          }
      });
